...
router.post('/message', async (req, res) => {
    try {
        ...
        const rasaResponse = await axios.post(`${process.env.BACKEND_NODEJS_URL}/webhooks/rest/webhook`, {
            sender,
            message,
        });
...
router.get('/status', async (req, res) => {
    ...
    if (rasaProcess) {
        try {
            const response = await axios.get(`${process.env.BACKEND_NODEJS_URL}/status`, { timeout: 5000 }); // 5-second timeout
